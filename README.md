
# Lee GyuYeol (이규열)
Frontend Developer

<br/>

## 📚 Career
- **xinics (2026.01 ~ 2026.05)**
- **zylo Systmes (2025.06 ~ 2025.09)**
- **Software Maestro (2024.03 ~ 2024.12)**
- **Dankook University (2022.03 ~ 2024.02)**

<br/>

## 💻 Projects
- **[edupi - 코드 시각화 교육 플랫폼 (2024.04 ~ 2024.12)](https://www.edupi.co.kr/)**
  
  코드를 분석하여 시각적으로 표현해주는 교육 플랫폼입니다.  
  AST 분석 결과를 기반으로 코드의 구조를 시각화하고,  
  학습자가 직관적으로 코드를 이해할 수 있도록 돕습니다.  
  프론트엔드는 React, Zustand, Framer Motion 등을 사용하여  
  인터랙티브하고 직관적인 사용자 경험을 구현했습니다.

<br/>

## 📊 Stats
<div>
  <a href="https://solved.ac/seroak/">
    <img src="https://mazassumnida.wtf/api/v2/generate_badge?boj=seroak" />
  </a>
  <img src="https://github-readme-stats.vercel.app/api?username=seroak&show_icons=true&theme=dracula" />
</div>

## 🛠 Tech Stack
![HTML5](https://img.shields.io/badge/html5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![React](https://img.shields.io/badge/react-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Zustand](https://img.shields.io/badge/zustand-000000?style=for-the-badge&logo=zustand&logoColor=white)
![React Query](https://img.shields.io/badge/reactquery-FF4154?style=for-the-badge&logo=reactquery&logoColor=white)
![Express](https://img.shields.io/badge/express-000000?style=for-the-badge&logo=express&logoColor=white)
![NestJS](https://img.shields.io/badge/nestjs-E0234E?style=for-the-badge&logo=nestjs&logoColor=white)
![Git](https://img.shields.io/badge/git-F05032?style=for-the-badge&logo=git&logoColor=white)

<br/>
